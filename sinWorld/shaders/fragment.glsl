...
float getWater(vec3 p) {
    float d = p.y + 0.5 * sin(u_time) + 70.0;
...
float getTerrain(vec3 p) {
    float d = 0;
    d -= 100.0 * noise(p.xz * 0.002);
    d += 75.0 * noise(p.xz * 0.01) + 50.0;
    d += 15.0 * fbm(p.xz * 0.1) * noise(p.xz * 0.01) + 20.0;
    d -= 2.0 * sin(0.6 * d);
    d += p.y + 2.0;
    return d * 0.1;
}
...
vec3 getLight(vec3 p, vec3 rd) {
    vec3 color = vec3(1); // 0.1, 0.23, 0.53
